           ...
           <table id="myTable" class="table table-bordered table-striped">
            ...
            <?php
                ...
                foreach ($sale_details as $saledet){
                  $selling.=$saledet->articles_id."-";
                }

                ?>
            <input id="selling" value="<?= $selling ?>" name="selling" readonly>
            Editando venta: <input id="id" name="id" value="<?= $sale->id ?>" readonly>
              <?php foreach ($articles as $article): ?>
                <?php
                $sub_quant=0;
                $sub_total=0;
                foreach ($sale_details as $saledet){
                  if($saledet->articles_id==$article->id){
                    $sub_quant=$saledet->quantity;
                    $sub_total=$saledet->total;
                    break;
                  }
                }
            ...
            <tr id="tr<?= h($article->id) ?>">
                  ...
                  <td><input onchange="update(this, <?= $article->referential_price ?>)" type="number" id="q<?= $article->id ?>" value="<?= $sub_quant ?>"  name="q<?= $article->id ?>"></td>
...
<?php echo $this->Html->script('newbuy'); ?>
<script>
updateStocks();
</script>
<?php $this->end(); ?>