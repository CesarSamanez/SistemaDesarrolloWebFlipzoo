            ...
            <tbody>
              <?php foreach ($tables as $table): ?>
                ...
                <tr>
                  ...
                  <td><span id="horaf<?= $table->id?>">Cuando quieras :v</td>
                  <td><span id="horas<?= $table->id?>"></td>
                  <td><input id="total<?= $table->id?>" type="number" step="0.10" value="0" min="0" ></td>
									<td><?= $this->Html->link(__('Iniciar'), ['controller'=> 'temporalUses', 'action' => 'update', $table->id	], ['class'=>'btn btn-info btn-xs']) ?>
									<?= $this->Html->link(__('Finalizar'), ['controller'=> 'tableUses', 'action' => 'add', $table->id, 1	], ['class'=>'btn btn-info btn-xs']) ?></td>
                  <td><button type="button" id="s<?= $table->id?>" class="btn btn-outline-success" onclick="cargaMesa(<?= $table->id?>)">Iniciar</button>
              		<button type="button" id="e<?= $table->id?>" disabled="true" class="btn btn-outline-warning" onclick="detenerMesa(<?= $table->id?>)">Finalizar</button></td>
                </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
