            ...
            <tbody>
              <?php foreach ($timbas as $timba): ?>
                ...
                <tr>
                  ...
                  <td><input type='number' name='people<?= $timba->id ?>' id='people<?= $timba->id ?>'  ></td>
                  ...
                  <td><input id="total<?= $timba->id?>" type="number" step="0.10" value="0" min="0" ></td>
									<td><?= $this->Html->link(__('Iniciar'), ['controller'=> 'TimbaTemporalUses', 'action' => 'update', $timba->id	], ['class'=>'btn btn-info btn-xs']) ?>
									<?= $this->Html->link(__('Finalizar'), ['controller'=> 'timbasUses', 'action' => 'add', $timba->id, 1	], ['class'=>'btn btn-info btn-xs']) ?></td>
                  <td><button type="button" id="s<?= $timba->id?>" class="btn btn-outline-success" onclick="cargaMesa(<?= $timba->id?>)">Iniciar</button>
              		<button type="button" id="e<?= $timba->id?>" disabled="true" class="btn btn-outline-warning" onclick="detenerMesa(<?= $timba->id?>)">Finalizar</button></td>
                </tr>
              <?php endforeach; ?>
            </tbody>
          </table>
